<template>
  <div class="sidebar">
    ...
    <div class="sidebar-item">
      <div class="sidebar-title" @click="toggleCategory('[data-lang]')">Язык</div>
      <label v-for="(language, i) in dataSidebar.language" :key="i">
        <input type="checkbox" :data-lang="language.lang" checked="checked" @click="isFilter($event)" />
...
export default {
    ...
    return {
      dataSidebar: {
        executor: [
          ...
          {name: 'Ноль', exec: 'nol'},
          {name: 'ГО', exec: 'go'},
          {name: 'Скрябин', exec: 'skryabin'},
          {name: 'Лявон Вольскі', exec: 'lyavon'},
          {name: 'Ляпис Трубецкой', exec: 'lyapis'},
          {name: 'Баста', exec: 'basta'},
          {name: 'Сразу Май', exec: 'srazu'},
          {name: 'Порнофильмы', exec: 'filmy'},
          {name: 'Король и шут', exec: 'korol'},
          {name: 'Тік', exec: 'tik'}
        ],
        language: [
          {name: 'Русский', lang: 'rus'},
          {name: 'Белорусский', lang: 'by'},
          {name: 'Украинский', lang: 'ua'}
        ],
        mood: [
      ...
      exec: ['tcoy', 'nol', 'go', 'skryabin', 'lyavon', 'lyapis', 'basta', 'srazu', 'filmy', 'korol', 'tik'],
      lang: ['rus', 'by', 'ua'],
  ...
  methods: {
    ...
    isFilter(v) {
      ...
      this.lang.indexOf(v.target.dataset.lang) != -1 ?
      this.lang.remove(v.target.dataset.lang) :
      this.lang.push(v.target.dataset.lang) 

      this.setFilter([this.exec, this.moo, this.lang]);
    },
    chooseAll() {
      this.exec = ['tcoy', 'nol', 'go', 'skryabin', 'lyavon', 'lyapis', 'basta', 'srazu', 'filmy', 'korol', 'tik'];
      this.lang = ['rus', 'by', 'ua'];
    ...
    toggleCategory(val) {
      ...
      dataCat.forEach(input => {
        val === '[data-exec]' ? this.exec = ['tcoy', 'nol', 'go', 'skryabin', 'lyavon', 'lyapis', 'basta', 'srazu', 'filmy', 'korol', 'tik'] : 
        val === '[data-lang]' ? this.lang = ['rus', 'by', 'ua'] : 
